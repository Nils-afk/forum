							...
							if (serverPlayerList === 1) {
								if (data.player_list.length > 0) {
									...
									for (var i = 0; i < data.player_list.length; i++) {
										serverPlayers += '<div class="list-item"><a href="' + URLBuild('profile/' + data.player_list[i].name) + '" data-toggle="tooltip" title="' + data.player_list[i].name + '"><img src="' + avatarSource.replace("{x}", data.player_list[i].name).replace("{y}", 64) + '" alt="' + data.player_list[i].name + '"></a></div>';
									}
